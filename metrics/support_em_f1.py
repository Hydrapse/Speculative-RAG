...
def compute_metrics(predicted_support: List[str], gold_support: List[str], is_fuzzy: bool = False) -> Dict[str, float]:
    # Exact match, taken from hotpot_eval
    if not is_fuzzy:
        ...
        for e in predicted_support:
            is_included = False
            for g in gold_support:
                if fuzz.partial_ratio(e, g) >= THRESHOLD:
                    is_included = True
                    # print(f"Fuzzy match: {e} ~ {g} (ratio: {fuzz.partial_ratio(e, g)})")
                else:
                    continue
            if is_included:
                tp += 1
                ...
                fp += 1
        for e in gold_support:
            matched = any(fuzz.partial_ratio(e, p) >= THRESHOLD for p in predicted_support)
            if not matched:
                fn += 1
...
class SupportEmF1Metric(Metric):
    ...
    def __init__(self, do_normalize_answer: bool = False, save_all: bool = False) -> None:
    ...
    def __call__(self, predicted_titles_and_paras, gold_titles_and_paras):
        
        if self._do_normalize_answer:
            if isinstance(predicted_titles_and_paras[0], list):
                predicted_titles_and_paras[0] = [normalize_answer(e) for e in predicted_titles_and_paras[0]]
                predicted_titles_and_paras[1] = [normalize_answer(e) for e in predicted_titles_and_paras[1]]
            else:
                predicted_titles_and_paras = [normalize_answer(e) for e in predicted_titles_and_paras]
            if isinstance(gold_titles_and_paras[0], list):
                gold_titles_and_paras[0] = [normalize_answer(e) for e in gold_titles_and_paras[0]]
                gold_titles_and_paras[1] = [normalize_answer(e) for e in gold_titles_and_paras[1]]
            else:
                gold_titles_and_paras = [normalize_answer(e) for e in gold_titles_and_paras]

        if predicted_titles_and_paras[0][0].startswith("pid"):
            for e in predicted_titles_and_paras[0] + gold_titles_and_paras[0]:
                assert e.startswith("pid")
            predicted_support_titles = [e.split("___")[1] for e in predicted_titles_and_paras[0]]
            predicted_support_paras = predicted_titles_and_paras[1]
            gold_support_titles = [e.split("___")[1] for e in gold_titles_and_paras[0]]
            gold_support_paras = gold_titles_and_paras[1]

        else:
            for e in predicted_titles_and_paras[0] + gold_titles_and_paras[0]:
                assert not e.startswith("pid")
            predicted_support_titles = predicted_titles_and_paras[0]
            predicted_support_paras = predicted_titles_and_paras[1]
            gold_support_titles = gold_titles_and_paras[0]
            gold_support_paras = gold_titles_and_paras[1]
        ...
        titles_metrics = compute_metrics(predicted_support_titles, gold_support_titles, is_fuzzy=False)
        paras_metrics = compute_metrics(predicted_support_paras, gold_support_paras, is_fuzzy=True)
        ...
        if self.save_all:
            self.all_scores["title_em"].append(titles_metrics["em"])
            self.all_scores["title_f1"].append(titles_metrics["f1"])
            self.all_scores["title_precision"].append(titles_metrics["prec"])
            self.all_scores["title_recall"].append(titles_metrics["recall"])
            self.all_scores["para_em"].append(paras_metrics["em"])
            self.all_scores["para_f1"].append(paras_metrics["f1"])
            self.all_scores["para_precision"].append(paras_metrics["prec"])
            self.all_scores["para_recall"].append(paras_metrics["recall"])
    ...
    def get_metric(self, reset: bool = False) -> Tuple[float, float]:
        ...
        return {
            "title_em": round(titles_exact_match, 4),
            "title_f1": round(titles_f1_score, 4),
            "title_precision": round(titles_precision_score, 4),
            "title_recall": round(titles_recall_score, 4),
            "para_em": round(paras_exact_match, 4),
            "para_f1": round(paras_f1_score, 4),
            "para_precision": round(paras_precision_score, 4),
            "para_recall": round(paras_recall_score, 4),
    ...
    def save_to_files(self, output_file: str):
        if self.save_all:
            with open(output_file, "w") as f:
                json.dump(self.all_scores, f)
        else:
            raise ValueError(
                "save_all was not set to True when calling the metric. Cannot save to file."
            )