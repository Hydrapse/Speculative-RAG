...
class DropAnswerEmAndF1(Metric):
    ...
    def __init__(self, save_all=False) -> None:
        self._total_em = 0.0
        self._total_f1 = 0.0
    ...
    def __call__(
        ...
        self._count += 1
        if self.save_all:
            self.all_scores["em"].append(exact_match)
            self.all_scores["f1"].append(f1_score)
            self.all_scores["precision"].append(prec_score)
            self.all_scores["recall"].append(recall_score)
